...
to setup-experiment
  ...
  [
    ...
    ifelse heterogeneity [
      ...
      set threshold avg-threshold  ;; probability to adopt innovation when others with innovation are in the cell
    ]
    set color white
  ]
...
ask turtles  
  [
        ...
        if sum0 + sum1 > 0
        [
        if (sum1 / (sum0 + sum1) > threshold) ;; draw random number and check whether it meets the threshold
            [ set color red;; if so adopt
             ] ]]
end
...
to plot-counts
  ...
  set adopted count turtles with [ color = red ]
...
to setup-plot
  set-current-plot "Adoption"
  set-plot-y-range 0 floor (groupsize * 1)
end
@#$#@#$#@
GRAPHICS-WINDOW
408
...
@#$#@#$#@
Threshold model of diffision processes by Marco Janssen, Arizona State University, 2010
@#$#@#$#@
default
true
0
