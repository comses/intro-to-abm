...
[
  ...
  connected?                           ;; are all nodes connected
...
to setup
  ...
  if Network? = "Small World Network" [
     make-turtles
     let success? false
     while [not success?] [
       wire-them
       ...
       do-calculations
       set success? connected?
     ]
     show connected?
     find-all-components
     color-giant-component
  ]
  if Network? = "Scale Free Network" [
...
to go
  ...
  do-calculations
  tick
  if Network? = "Random Network" [if giant-component-size = num-nodes [stop]]
  if Network? = "Small World Network" [stop]
  if Network? = "Scale Free Network" [if count turtles = num-nodes [stop]]
end
