...
to setup
  ...
  rewire-all
  ask one-of turtles [set adopted 1 set color red]
end

to make-turtles
  crt num-nodes [ 
    if distribution = "allthesame" [set threshold thresholdATS]
    if distribution = "gaussian" [
      let found 0
      while [found = 0]
      [
        set threshold random-normal 0.5 0.25
        if threshold >= 0 and threshold <= 1 [set found 1]
      ]
    ]
    if distribution = "uniform" [set threshold random-float 1]    
    set color gray + 2 
  ]
...
to-report do-calculations
  ...
  if ( num-connected-pairs != (count turtles * (count turtles - 1) ))
  [
...
to go 
  ask turtles [set desadopted 0]
  ask turtles [
    set obsshare count link-neighbors with [adopted = 1] / count link-neighbors
    if obsshare > threshold [set desadopted 1]
  ]
  ask turtles [
    if desadopted = 1 [set adopted 1]
    if adopted = 1 [set color red]
  ]
  tick
end
...
<experiments>
  <experiment name="experiment" repetitions="1000" runMetricsEveryStep="false">
    ...
    <enumeratedValueSet variable="thresholdATS">
      <value value="0.5"/>
    </enumeratedValueSet>
